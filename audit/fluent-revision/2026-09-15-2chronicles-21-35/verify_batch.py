...
BASE='498038a0fe190418dfbf27f2bbc54add7a8db8d5'
...
def main():
 ...
 assert len(scopes)==65 and len(paths)==407 and total==12102 and old_chapters==392
 ...
 for sc,bk,pre,start,end,bookqa in zip(scopes[-1:],['2chronicles'],['2Chronicles'],[21],[35],['2_CHRONICLES_FLUENT_BOOK_QA.json']):
  ...
  assert len(bound)=={'2chronicles':369}[bk]
  focused[bk]=len(rows)
 assert focused=={'2chronicles':89}
 for c in range(21,36):
  for v in ledgers[-1]['verses']:
   if v['reference'].startswith(f'2 Chronicles {c}:'):assert v['source_reference']=='2Chr '+v['reference'].split()[-1]
 ...
 assert 'Ahaziah son' not in allv['2 Chronicles 22:6']['after']
 assert 'gave them rest' not in allv['2 Chronicles 32:22']['after']
 assert 'thirty years' not in allv['2 Chronicles 31:16']['after']
 ...
 result={'status':'PASSED','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'base_commit':BASE,'scope':'2 Chronicles 21–35','new_chapters':15,'new_verses':369,'affected_source_binding_audits':audits,'preservation':{'prior_ledgers_byte_identical':64,'prior_chapters_byte_identical':392,'historical_provenance_and_unrelated_work_preserved':True},'cumulative_coverage':{'ledgers':65,'chapters':407,'verses':12102},'structural_chapters':structural,'first_source_reading':{'public_verses':369,'original_records':369,'all_original_annotations_read':True},'tsw_comparator_reading':369,'focused_source_reread':focused,'focused_assertions':checks,'whole_1chronicles_source_records_bound_once':943,'selected_2chronicles_source_records_bound_once':369,'public_source_versification_checked':True,'translation_family_audit':run(sys.executable,'tools/audit_translation_family.py'),'git_diff_check':run('git','diff','--check'),'independent_editorial_review':'REVIEW_PENDING','whole_book_samuel_kings_and_1chronicles_consistency_passes':'PENDING','reader_testing':'PENDING','publication_allowed':False}
