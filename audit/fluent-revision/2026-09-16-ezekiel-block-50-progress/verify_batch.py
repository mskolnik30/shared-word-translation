...
ROOT=Path(__file__).resolve().parents[3];BATCH=Path(__file__).resolve().parent;BASE='e1238360da0bbce966d2d56162ed480fd570f273'
...
C=[('ezekiel','Ezekiel','Ezekiel',2,1,24,619,136)]
...
def main():
 ...
 assert len(oq['completed_draft_scopes'])==88 and len(priorpaths)==807 and total==20572
 ...
 checks={'Ezekiel 1:1':['thirtieth','fourth','fifth','Kebar'],'Ezekiel 2:3':['nations'],'Ezekiel 4:5':['390'],'Ezekiel 4:6':['forty'],'Ezekiel 4:9':['wheat','barley','beans','lentils','millet','spelt'],'Ezekiel 4:10':['twenty shekels'],'Ezekiel 4:11':['sixth of a hin'],'Ezekiel 4:15':['cattle dung'],'Ezekiel 8:2':['fire'],'Ezekiel 9:6':['children'],'Ezekiel 10:14':['cherub'],'Ezekiel 11:19':['one heart','within you'],'Ezekiel 14:9':['I, the LORD, have deceived'],'Ezekiel 16:49':['pride','poor','needy'],'Ezekiel 19:7':['his widows'],'Ezekiel 19:10':['your blood'],'Ezekiel 20:25':['not good'],'Ezekiel 21:17':['I too'],'Ezekiel 22:25':['prophets'],'Ezekiel 23:20':['sexual organs','donkeys','emissions','horses'],'Ezekiel 24:1':['ninth','tenth'],'Ezekiel 24:18':['my wife died']}
 for ref,terms in checks.items():
  for term in terms:assert term.lower()in allvv[ref]['after'].lower(),(ref,term)
 for row in allbooks['ezekiel']['verses']:
  c,v=map(int,row['reference'].split()[-1].split(':'));sc,sv=(21,v-44)if c==20 and v>=45 else ((21,v+5)if c==21 else(c,v));assert row['source_reference']==f'Ezek {sc}:{sv}'
 assert len(q['completed_draft_scopes'])==89
 cumulative=[ch for sc in q['completed_draft_scopes']for ch in json.loads((ROOT/sc['ledger']).read_text())['chapters']]
 assert len(cumulative)==831 and sum(ch['verse_count']for ch in cumulative)==21191
 block=q['active_fifty_chapter_block'];assert block['status']=='IN_PROGRESS'and block['completed_chapters']==24 and block['remaining_chapters']==26 and block['completed_verses']==619
 ...
 result={'status':'PASSED','base_commit':BASE,'new_chapters':24,'new_verses':619,'unique_original_source_records':sourcecounts,'cumulative_coverage':{'ledgers':89,'chapters':831,'verses':21191},'prior_preservation':{'ledgers_byte_identical':88,'chapters_byte_identical':807,'verses':20572,'tsw_unchanged':True,'historical_provenance_preserved':True},'source_binding_audit':'PASSED for affected Ezekiel ledger','divine_names':'PASSED verse by verse, including LORD/GOD and YAH separately','focused_assertions':checks,'versification':'619 unique exact original records; public 20:45–49 maps to Hebrew21:1–5 and public21:1–32 maps to Hebrew21:6–37.','repeated_refrains':'Divine-name repetitions checked verse by verse; recurring promise and judgment phrases reread.','focused_authoring_reread':136,'translation_family':family,'git_diff_check':diff,'independent_editorial_review':'REVIEW_PENDING','reader_testing':'PENDING','whole_book_consistency_reviews':'All prior pending reviews preserved.','active_fifty_chapter_block':block,'publication_allowed':False}
