...
BASE='625643519e5a0bd0d812b0add13989028216c6c6'
...
def main():
 ...
 assert len(scopes)==64 and len(paths)==392 and total==11733 and old_chapters==377
 ...
 for sc,bk,pre,start,end,bookqa in zip(scopes[-1:],['2chronicles'],['2Chronicles'],[6],[20],['2_CHRONICLES_FLUENT_BOOK_QA.json']):
  ...
  assert len(bound)=={'2chronicles':342}[bk]
  focused[bk]=len(rows)
 assert focused=={'2chronicles':73}
 assert allv['2 Chronicles 14:1']['source_reference']=='2Chr 13:23'
 for n in range(2,16):assert allv[f'2 Chronicles 14:{n}']['source_reference']==f'2Chr 14:{n-1}'
 ...
 assert 'my father made' not in allv['2 Chronicles 10:14']['after'].casefold()
 assert 'Edom' not in allv['2 Chronicles 20:2']['after']
 assert 'clothing' not in allv['2 Chronicles 20:25']['after']
 for c,vs in [(6,[2,41,42]),(10,[16]),(18,[16])]:
  text=(ROOT/f'translations/fluent/OT/2chronicles/2Chronicles_{c:02}.md').read_text()
  for v in vs:
 ...
 result={'status':'PASSED','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'base_commit':BASE,'scope':'2 Chronicles 6–20','new_chapters':15,'new_verses':342,'affected_source_binding_audits':audits,'preservation':{'prior_ledgers_byte_identical':63,'prior_chapters_byte_identical':377,'historical_provenance_and_unrelated_work_preserved':True},'cumulative_coverage':{'ledgers':64,'chapters':392,'verses':11733},'structural_chapters':structural,'first_source_reading':{'public_verses':342,'original_records':342,'all_original_annotations_read':True},'tsw_comparator_reading':342,'focused_source_reread':focused,'focused_assertions':checks,'whole_1chronicles_source_records_bound_once':943,'selected_2chronicles_source_records_bound_once':342,'public_source_versification_checked':True,'translation_family_audit':run(sys.executable,'tools/audit_translation_family.py'),'git_diff_check':run('git','diff','--check'),'independent_editorial_review':'REVIEW_PENDING','whole_book_samuel_kings_and_1chronicles_consistency_passes':'PENDING','reader_testing':'PENDING','publication_allowed':False}
